...
export SUBNET_ID=$(aws ec2 describe-subnets --filters "Name=vpc-id,Values=${QUOTED_VPC_ID}" "Name=availability-zone,Values=${AWS_REGION}a" --query "Subnets[?MapPublicIpOnLaunch==\`true\`].SubnetId | [0]" --output text) && echo $SUBNET_ID
