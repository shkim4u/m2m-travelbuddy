variable "chat_api_lambda_function_invoke_arn" {}
variable "chat_api_lambda_function_name" {}
variable "upload_lambda_function_invoke_arn" {}
variable "upload_lambda_function_name" {}
variable "query_result_lambda_function_invoke_arn" {}
variable "query_result_lambda_function_name" {}
variable "history_lambda_function_invoke_arn" {}
variable "history_lambda_function_name" {}
variable "delete_log_lambda_function_invoke_arn" {}
variable "delete_log_lambda_function_name" {}
variable "chat_ws_lambda_function_invoke_arn" {}
variable "chat_ws_lambda_function_name" {}
variable "chat_ws_lambda_function_alias_name" {}

variable "prompt_api_lambda_function_invoke_arn" {}
variable "prompt_api_lambda_function_arn" {}
variable "prompt_api_lambda_function_name" {}
variable "prompt_api_lambda_function_alias_name" {}
